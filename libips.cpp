...
#define read24() (patchat += 3, ((patchat[-3] << 16) | (patchat[-2] << 8) | patchat[-1]))
	...
	while (offset != 0x454F46)
	{
		...
		if (size == 0)
		{
			...
			if (size && (out->ptr[offset] != b || memcmp(out->ptr + offset, out->ptr + offset, size - 1)))
				anychanges = true;
		...
		{
			if (memcmp(out->ptr + offset, patchat, size))
				anychanges = true;
...
#undef read24
	...
	if (study->outlen_max != 0xFFFFFFFF && in.len <= study->outlen_max)
		study->error = ips_notthis; //truncate data without this being needed is a poor idea
	if (!anychanges)
...
enum ipserror ips_create(struct mem sourcemem, struct mem targetmem, struct mem *patchmem)
{
	...
	if ((targetlen != sourcelen && targetlen > 0xFFFFFF) || (targetlen > 0x1000000))
	{
		return ips_16MB;
	}
...
#define read24() ((patchat + 2 < patchend) ? (patchat += 3, ((patchat[-3] << 16) | (patchat[-2] << 8) | patchat[-1])) : 0)
	...
	while (offset!=0x454F46)//0x454F46="EOF"
	{
		int size=read16();
